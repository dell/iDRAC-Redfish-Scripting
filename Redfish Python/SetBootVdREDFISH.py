...
parser.add_argument('script_examples',action="store_true",help='SetBootVdREDFISH.py -ip 192.168.0.120 -u root -p calvin -c y, this example will get current server controller FQDDs. SetBootVdREDFISH.py -ip 192.168.0.120 -u root -p calvin -gb RAID.Slot.6-1, this example will get current boot VD value for controller RAID.Slot.6-1. SetBootVdREDFISH.py -ip 192.168.0.120 -u root -p calvin -bc RAID.Slot.6-1 -bv Disk.Virtual.0:RAID.Slot.6-1, this example will set VD 0 as boot VD for controller RAID.Slot.6-1 ')
...
parser.add_argument('-gb', help='Get the current virtual disk set as bootable for the controller, pass in storage controller FQDD, Example "\RAID.Integrated.1-1\"', required=False)
...
parser.add_argument('-bc', help='Pass in controller FQDD you want to set the boot VD, Example "\RAID.Mezzanine.1-1\". NOTE: Arguments -bc and -bv are both required to set the controller boot VD', required=False)
parser.add_argument('-bv', help='Pass in VD FQDD you want to set as bootable for the controller, Example "\Disk.Virtual.0:RAID.Mezzanine.1-1\"', required=False)
...
def set_boot_vd():
    ...
    payload={"ControllerFQDD":args["bc"], "VirtualDiskFQDD":args["bv"]}
    ...
    if data['JobType'] == "RAIDConfiguration":
        ...
        job_type="realtime"
    print("\n- PASS, \"%s\" %s jid successfully created for setting boot VD\n" % (job_type, job_id))
...
def check_boot_vd_final_status():
    response = requests.get('https://%s/redfish/v1/Systems/System.Embedded.1/Storage/%s' % (idrac_ip, args["bc"]),verify=False,auth=(idrac_username, idrac_password))
    data = response.json()
    for i in data.items():
        if i[0] == "Oem":
            if args["bv"] == i[1]["Dell"]["DellController"]["BootVirtualDiskFQDD"]:
                print("\n- PASS, VD \"%s\" successfully set as boot VD for controller \"%s\"" % (args["bv"], args["bc"]))
            else:
                print("\n- FAIL, VD %s NOT set as boot VD for controller %s, current value is: %s" % (args["bv"], args["bc"], i[1]["Dell"]["DellController"]["BootVirtualDiskFQDD"]))
                sys.exit()
...
if __name__ == "__main__":
    ...
    elif args["bc"] and args["bv"]:
        set_boot_vd()
        if job_type == "realtime":
            loop_job_status()
            check_boot_vd_final_status()
