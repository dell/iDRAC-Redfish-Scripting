...
if response.status_code != 200:
    print("- FAIL, GET request failed to get current iDRAC firmware version, status code %s returned, detailed error results: \n%s" % (response.status_code, data))
    sys.exit()
else:
    idrac_release_version = data[u'FirmwareVersion'].split(".")[:2]
    idrac_release_version = int("".join(idrac_release_version))
