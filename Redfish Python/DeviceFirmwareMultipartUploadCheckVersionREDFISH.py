...
parser=argparse.ArgumentParser(description="Python script using Redfish API to first check current firmware version for a device, then only apply the new firmware package if version difference is detected. NOTE: Pass in --force argument to apply the firmware even if same version is detected.")
...
parser.add_argument('--force', help='Pass in this argument to force the update when same device version installed and package version are the same.', action="store_true", required=False)

args=vars(parser.parse_args())
logging.basicConfig(format='%(message)s', stream=sys.stdout, level=logging.INFO)

def script_examples():
    print("""\n- DeviceFirmwareMultipartUploadCheckVersionREDFISH.py -ip 192.168.0.120 -u root -p calvin --get, this example will get current firmware versions for all devices in the server.
    \n- DeviceFirmwareMultipartUploadCheckVersionREDFISH.py -ip 192.168.0.120 -u root -p calvin --location C:\\Users\\administrator\\Downloads\\BIOS_2.11.2.EXE --reboot --force, this example will reboot the server now and force BIOS update if same version is first detected.
    """)
    sys.exit(0)
    
def check_supported_idrac_version():
    """ Check iDRAC version installed does support this workflow """
...
def extract_exe_and_find_package_xml(exe_file):
    """ Extract EXE file contents and locate package.xml file """
    ...
    try:
        ...
        logging.info("\n- INFO, getting current firmware version for DUP package \"%s\"" % args["location"].split("\\")[-1])
        try:
...
def get_FW_inventory_same_version_check(current_version_detected):
    global same_version_flag
    ...
    if same_version_flag == "yes":
        logging.info("- WARNING, DUP firmware version %s already installed for device %s" % (current_version_detected, same_version_device))
    else:
        logging.info("- INFO, DUP %s firmware version not installed for any device" % current_version_detected)
...
if __name__ == "__main__":
    ...
    if args["get"]:
        ...
        get_idrac_version()
        DUP_version = extract_exe_and_find_package_xml(args["location"])
        get_FW_inventory_same_version_check(DUP_version)
        if same_version_flag == "yes" and args["force"]:
            logging.info("- INFO, same version for device and argument force both detected, update will be applied")
        elif same_version_flag == "no" and args["force"]:
            logging.info("- INFO, same version for device not detected and argument force detected, update will be applied")
        elif same_version_flag == "yes" and not args["force"]:
            logging.info("- INFO, same version for device detected and argument force not detected, update will NOT be applied")
            sys.exit(0)
        else:
            logging.info("- INFO, same version for device not detected and argument force not detected, update will still be applied")
