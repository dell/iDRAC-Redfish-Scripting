...
parser.add_argument('-c', help='Pass in \"y\" to get current next boot onetime boot setting and possible values', required=False)
parser.add_argument('-o', help='Pass in the string onetime boot device you want to set for next reboot. NOTE: This value is case sensitive so pass in exact value as stated in possible values for -c option', required=False, type=str)
parser.add_argument('-U', help='Set UEFI target path. This will be used with -o option if you pass in UefiTarget value. ', required=False)
parser.add_argument('-g', help='Get UEFI target path values for each boot order entry, pass in \"y\".', required=False)
...
### Function to get UEFI target path info for boot order devices

def get_uefi_target_path_values():
    response = requests.get('https://%s/redfish/v1/Systems/System.Embedded.1/BootOptions?$expand=*($levels=1)' % idrac_ip,verify=False,auth=(idrac_username,idrac_password))
    ...
    data = response.json()
    if data["Members"] == []:
        print("- WARNING, no boot devices detected in the boot order")
        sys.exit()
    print("\n")
...
if __name__ == "__main__":
    ...
    if args["r"] == "n":
        print("- INFO, no reboot selected for -r argument. Onetime boot will be applied on next manual server reboot")
        sys.exit()
    elif args["r"] == "y":
        reboot_server()
