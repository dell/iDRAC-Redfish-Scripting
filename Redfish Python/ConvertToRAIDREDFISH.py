...
parser=argparse.ArgumentParser(description="Python script using Redfish API with OEM extension to convert drives to RAID or ready state")
...
parser.add_argument('--convert-raid', help='Convert drive to RAID(ready state), pass in the disk FQDD, Example \"Disk Disk.Bay.4:Enclosure.Internal.0-1:RAID.Slot.6-1\"', dest="convert_raid", required=False)

args=vars(parser.parse_args())
logging.basicConfig(format='%(message)s', stream=sys.stdout, level=logging.INFO)

def script_examples():
    print("""\n- ConvertToRAIDREDFISH.py -ip 192.168.0.120 -u root -p calvin --get-controllers, this example will return storage controller FQDDs detected.
    \n- ConvertToRAIDREDFISH.py -ip 192.168.0.120 -u root --get-disks RAID.Mezzanine.1-1, this example will first prompt to enter iDRAC user password, then get current disks and raidstatus for each disk behind controller RAID.Mezzanine.1-1.
    \n- ConvertToRAIDREDFISH.py -ip 192.168.0.120 -u root -p calvin --convert-raid  Disk.Bay.0:Enclosure.Internal.0-1:RAID.Mezzanine.1-1, this example is going to convert drive in bay 0 from not ready state to RAID (ready) state.""")
    sys.exit(0)
...
def convert_drives_RAID():
    global job_id
    method = "ConvertToRAID"
    url = 'https://%s/redfish/v1/Systems/System.Embedded.1/Oem/Dell/DellRaidService/Actions/DellRaidService.ConvertToRAID' % (idrac_ip)
    if "," in args["convert_raid"]:
        disks = args["convert_raid"].split(",")
    else:
        disks = [args["convert_raid"]]
    payload={"PDArray": disks}
    if args["x"]:
    ...
    if response.status_code == 200 or response.status_code == 202:
        logging.info("\n- PASS: POST command passed to convert disk \"%s\" to RAID, status code 200 returned" % args["convert_raid"])
        try:
        ...
        logging.error("\n- FAIL, POST command failed to convert disk \"%s\" to RAID, status code %s" % (args["convert_raid"], response.status_code))
        data = response.json()
