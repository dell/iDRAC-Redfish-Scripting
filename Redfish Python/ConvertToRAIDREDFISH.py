...
parser.add_argument('script_examples',action="store_true",help='ConvertToRAIDREDFISH.py -ip 192.168.0.120 -u root -p calvin -n Disk.Bay.2:Enclosure.Internal.0-1:RAID.Mezzanine.1-1, this example will convert disk 2 from non RAID to RAID(ready) state.')
...
parser.add_argument('-n', help='Convert drive to RAID(ready state), pass in the disk FQDD, Example \"Disk Disk.Bay.4:Enclosure.Internal.0-1:RAID.Slot.6-1\"', required=False)
...
def convert_drives_RAID():
    ...
    if response.status_code == 200 or response.status_code == 202:
        print("\n-PASS: POST command passed to convert disk \"%s\" to RAID, status code 200 returned" % args["n"])
        try:
