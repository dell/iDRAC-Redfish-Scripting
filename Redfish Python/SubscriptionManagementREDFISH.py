#
# SubscriptionManagementREDFISH. Python script using Redfish API to either get event service properties, get event subscriptions, create / delete subscriptions or submit test event.
#
...
parser.add_argument('script_examples',action="store_true",help='SubscriptionManagementREDFISH.py -ip 192.168.0.120 -u root -p calvin -s yy, this example will get current subscription URIs and details. SubscriptionManagementREDFISH.py -ip 192.168.0.120 -u root -p calvin -c y -D https://192.168.0.130 -E Alert -V MetricReport, this example will create a MetricReport subscription for alert events which will use 192.168.0.130 Redfish event listener. SubscriptionManagementREDFISH.py -ip 192.168.0.120 -u root -p calvin --delete /redfish/v1/EventService/Subscriptions/c1a71140-ba1d-11e9-842f-d094662a05e6, this example will delete a subscription')
...
parser.add_argument('-E', help='Pass in EventType value for either create subscription or send test event. Supported values are StatusChange, ResourceUpdated, ResourceAdded, ResourceRemoved, Alert, MetricReport.', required=False)
