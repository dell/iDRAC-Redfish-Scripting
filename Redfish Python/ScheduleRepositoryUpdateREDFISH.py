...
parser.add_argument('--dayofweek', help='Specify day of week on which the update is scheduled. The possible values are * (Any), Mon, Tue, Wed, Thu, Fri, Sat, Sun. The default value is *. Note: --dayofweek and --dayofmonth are mutually exclusive arguments, only pass in one for setting update schedule.',required=False)
parser.add_argument('--dayofmonth', help='Specify day of month on which the update is scheduled. The possible values are * (Any) or a number between 1-28. The default value is *. Note: --dayofweek and --dayofmonth are mutually exclusive arguments, only pass in one for setting update schedule.',required=False)
