...
<#
.Synopsis
   Cmdlet used to either get storage controllers, get virtual disks or init virtual disk
.DESCRIPTION
   Cmdlet used to either get storage controllers, get virtual disks or initialize virtual disk using iDRAC Redfish API.
   ...
   - init_virtual_disk: Pass in the virtual disk FQDD to delete. Example, pass in "Disk.Virtual.0:RAID.Slot.6-1" to init virtual disk 0 for controller RAID.Slot.6-1
   - init_type: Pass in the init type for the virtual disk. Supported values are "Fast" and "Slow". Note, certain controllers might not support both values. Check the controller user guide for more information if needed.

.EXAMPLE
   .\Invoke-InitVirtualDiskREDFISH -idrac_ip 192.168.0.120 -username root -password calvin -get_storage_controllers y
   This example will return storage controller FQDDs for the server.
.EXAMPLE
   .\Invoke-InitVirtualDiskREDFISH -idrac_ip 192.168.0.120 -username root -password calvin -init_virtual_disk Disk.Virtual.0:RAID.Slot.6-1 -init_type Fast
   This example will run fast initialize on VD Disk.Virtual.0:RAID.Slot.6-1.
#>

function Invoke-InitializeVirtualDiskREDFISH {
...
if ($init_virtual_disk -and $init_type)
{
$uri = "https://$idrac_ip/redfish/v1/Systems/System.Embedded.1/Storage/Volumes/$init_virtual_disk/Actions/Volume.Initialize"
...
Write-Host "- WARNING, initialize virtual disk job will run in real time operation, no server reboot needed to apply the changes"
}
if ($overall_job_output.JobType -eq "RAIDConfiguration")
{
$job_type = "staged_config"
Write-Host "- WARNING, check initialize virtual disk job will run in staged operation, server reboot needed to apply the changes"
}
...
$uri = "https://$idrac_ip/redfish/v1/Systems/System.Embedded.1/Storage/Volumes/$init_virtual_disk"
try
    {
    if ($global:get_powershell_version -gt 5)
    {
...
if ($init_type -eq "Fast")
{
if ($operation_value.Length -eq 0)
{
Write-Host "`n- PASS, fast init operation has completed successfully"
return
}
else
{
Write-Host "`n- FAIL, fast init not completed, progress is: $operation_value"
return
}
}
...
$get_init_progress = [regex]::Match($content, "OperationName.+?]").captures.groups[0].value
$get_init_progress = $get_init_progress.Replace("}]","")
$current_init_progress=$get_init_progress.Split(",")
$operation_value = $get_result.Operations

Start-Sleep 10

if ($init_type -eq "Slow")
{
if ($operation_value.Length -gt 0)
{
Write-Host "`n- PASS, slow init operation has started"
return
}
else
{
Write-Host "`n- FAIL, slow init not in progess, detailed information: $operation_value"
return
}
}
...
$get_content = $result.Content | ConvertFrom-Json
$content = $result.Content 
$get_init_status = [regex]::Match($content, "OperationName.+?]").captures.groups[0].value
$get_init_status = $get_init_status.Replace("}]","")
$current_init_progress = $get_init_status.Split(",")
$operation_value = $get_content.Operations
